...
std::string Polygon::svg (bool rel) const
{
    std::stringstream ss;
    Dim from{};
    const Dim O;
...
void Polygon::push (Dim d, bool tidy)
{
...
struct BorderFinder
{
    ...
    BorderFinder (const Cubes &cs)
        : cs(cs)
    {
        Assert (DIM == 2, "BorderFinder doesn't work in DIM %d", DIM);
    ...
    Polygon get_polygon (bool outer, bool tidy)
    {
        ...
        for (Line line = start; ; line = Line { line.b, p2 })
        {
            ...
            for (Line l : *buck)
            {
                if (l.a != line.b)
                    std::swap (l.a, l.b);
                Assert (l.a == line.b, "line b must continue line a");
    ...
    Polygons border (bool tidy = 1)
    {
        ...
        Assert (polygons.size () >= 1, "border has no lines");
        return polygons;
    }
};

Polygons cubes_border (const Cubes &c)
{
    return BorderFinder (c).border ();
}

std::ostream& operator << (std::ostream &ost, Line l)
{
    return ost << l.a << "--" << l.b;
}
...
std::ostream& operator << (std::ostream &ost, const Polygon &pg)
{
    ost << "Polygon: ";
    for (Dim p : pg)
        ost << " " << p;
